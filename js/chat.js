...
var ajaxChat = {
	...
	getUserNodeStringItems: function(encodedUserName, userID) {
		...
		if(encodedUserName != this.encodedUserName) {
			...
			if(this.userRole == 3) {
				menu	+= '<li><a href="javascript:ajaxChat.insertMessageWrapper(\'/password '
						+ encodedUserName
						+ ' \');">'
						+ this.lang['userMenuPassword']
						+ '</a></li>';
			}
		} else {
			menu 	= '<li><a href="javascript:ajaxChat.sendMessageWrapper(\'/quit\');">'
					...
					+ '</a></li>';
			if(this.authMethod == 'database' && this.userRole != 0) {
				menu	+= '<li><a href="javascript:ajaxChat.insertMessageWrapper(\'/password \');">'
					+ this.lang['userMenuPassword']
					+ '</a></li>';
			}
	...
	replaceCommands: function(text) {
		try {
			...
			switch(textParts[0]) {
				...
				case '/password':
					return this.replaceCommandPassword(textParts);
				case '/error':
	...
	replaceCommandPassword: function(textParts) {
		return '<span class="chatBotMessage">'
				+ this.lang['passwordUpdated'].replace(/%s/, textParts[1])
				+ '</span>';
	},
